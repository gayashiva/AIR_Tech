...
The software uses several assumptions assumptions to reduce the data requirement of the energy balance model.
The assumptions are categorised into two kinds based on whether they favour higher water use efficiency or ice
...
recommeded discharge rate rates using the following set of equations:
...
where $r_F$ is the fountain spray radius, $Q_{F}$ is the discharge rate and $hod$ is the hour of day.
...
certain termination criterias override the discharge rate recommendation, namely: 
...
The performance of the ICV and WUE versions of the physical model was assessed by comparing its estimates with
the validated freezing rate of the calibrated physical model.
...
recommended discharge rate. Nevertheless, this model was able to capture ? \% of the freezing rate variations
