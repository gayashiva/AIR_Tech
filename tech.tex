...
where $T_{F}$ is the fountain water temperature parameter.
...
Most of the the default values of all parameters used in
\cite{balasubramanianInfluenceMeteorologicalConditions2022} for our analysis except the precipitation
...
since the unscheduled fountain's discharge duration and quantity were ? and ? times higher respectively.
...
surface is affected in the following three ways, namely, (a) its albedo dampens to ice albedo and (b) it absorbs
...
radiation fluxes contributed 0 \% and 14 \% to the melting energies for the traditional and automated AIR,
respectively.  Similarly, the turbulent fluxes contribute -10 \% and -8 \% to the freezing energies for the
traditional and automated AIR, respectively. The fountain water and ground heat flux contribute 10 \% and 2 \%
to the melting energies for the traditional and automated AIR, respectively. 

There is a considerable difference in the contribution of the shortwave radiation and fountain discharge heat
flux between the two AIRs. In particular, the shortwave radiation is higher for the automated AIR due to the
effect of process (a). Similarly, the fountain discharge heat flux for the traditional AIR is enhanced due to
process (b).
  ...
  simulation duration for the automated and the traditional AIRs}
	...
	\begin{tabular}{@{}|llllll|@{}}
		...
		\multicolumn{1}{|l|}{\multirow{7}{*}{\rotatebox[origin=c]{90}{Energy Balance}}}
                           ...
                           & Shortwave radiation             &  $q_{SW}$       & $33$  & $45$ & \% \\
		\multicolumn{1}{|l|}{} & Longwave radiation              &  $q_{LW}$       & $-33$  & $-31$ & \% \\
		\multicolumn{1}{|l|}{} & Sensible heat                   &  $q_{S}$        & $7$   & $7$ & \% \\
		\multicolumn{1}{|l|}{} & Latent heat                     &  $q_{L}$        & $-17$  & $-15$ & \% \\
		\multicolumn{1}{|l|}{} & Fountain discharge heat         &  $q_{F}$        & $9$  & $1$     & \% \\
		\multicolumn{1}{|l|}{} & Ground heat                     &  $q_{G}$        & $1$   & $1$     & \% \\\midrule
		\multicolumn{1}{|l|}{\multirow{2}{*}{\rotatebox[origin=c]{90}{AIR}}}

		                       & Maximum Ice Volume              &                 & 44            & 55            & $m^{3}$ \\
		\multicolumn{1}{|l|}{} & Water Use Efficiency            &                 & 6             & 33            & \% \\
		\multicolumn{1}{|l|}{} & Surface Area                    &  $A_{cone}$     & $54\pm 7$  & $70 \pm 14$     &$m^{2}$ \\\midrule
	\end{tabular}
\end{table}
...
Discharge rate was observed to increase from $Q_{i}$ = 11 $l/min$ to $Q_{f}$ = 27 $l/min$ upon removal of the fountain. Inputting
$P_{f} = 0$ and $h_{i} = h_{f}$ in Eqn. \ref{eqn:fountain} we get: 

\begin{equation}
  P_{i} = 1/2 \cdot \rho \cdot (Q_f/A)_f^2 - 1/2 \cdot \rho_{water} \cdot (Q_i/A)^2
\end{equation}

Inputting the corresponding values, we estimate $P$ to be around 0.19 $bar$. In other words, the pressure loss
occuring due to the fountain was equivalent to 1.9 meters of water head.

\section{Discussion}

\subsection{Fountain freezing events}

Both the scheduled and the unscheduled fountain were interrupted by a freezing event. However, the cause of the
freezing event was different in each of the cases. For the scheduled fountain, improper drainage of water
triggered a pipeline freezing event in one section of the pipe. For the unscheduled fountain, a fountain
freezing event froze the whole pipe. This fountain freezing event was triggered almost immediately after the
fountain height was increased even though the fountain discharge had attained the critical discharge rate as
shown in Fig. \ref{fig:aws}. Therefore both the freezing events could have been prevented by ensuring proper
drainage and avoiding fountain height increase events after attaining critical discharge rate. 

\subsection{Fountain design}

The unscheduled fountain was observed to produce bigger water droplets with different trajectories compared to
the scheduled fountain. This behaviour was due to the different engineering design of the two fountains as shown
in Fig. ?.
...
Fix fountain height, make multiple AIRs
...
fountain discharge rate not just increased the fountain wastewater production but also enhanced the melting rate of
...
favourable weather windows and prevents fountain discharge rate to lower below a critical threshold. Although,
