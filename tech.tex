...
\begin{abstract}
  ...
  improvement in water use efficiency that can be achieved by optimising the fountain discharge rate. To
  generate the fountain scheduling requirements for the automation software, a new empirical model that computes
  AIR freezing rates was developed. The automation hardware consisted of a weather station, flowmeter, control
  valve, drain valves, air valves and a logger. During the winter of 2021-22, a traditional and an automated AIR
  were built in Guttannen with the main aim of comparing and quantifying the benefits of using automation
  ...
  inspite of the reduced discharge rates. However, this volume difference was due to the better design of the
  automated fountain which produced a higher spray radius. Overall, our results show that the automated fountain
  scheduling method increased the water use efficiency of AIR construction 5 fold.
...
automation software and examples of its application to the computation of spatially distributed AIR construction
requirements. 
...
during the winters of 2020-21 using different fountain discharge strategies namely manual and dynamic as shown
in Fig. \ref{fig:2AIR}.
...
\subsection{Manual AIR construction strategy}

In the manual strategy, tree branches were laid covering the fountain pipe to initiate the ice formation
...
In the dynamic strategy, only the fountain pipe was erected before initiation of water spray. An automation
system controlled the fountain discharge rate using real time weather input. The automation hardware consists of
a weather station, flowmeter, control valve, drain valves, air valves, fountain, pipeline and a logger. The
logger feeds the weather station data to the automation software and informs the recommended discharge rate to
the flowmeter. The flowmeter adjusts the control valve to match the recommendation. In case the recommendation
is below the critical discharge rate of the fountain, the drain and air valves empty the water in the pipeline
to prevent any freezing events. Here the critical discharge rate is defined as the discharge rate below which
the fountain pipeline can freeze.
...
balance model used in \cite{balasubramanianInfluenceMeteorologicalConditions2022} and
\cite{oerlemansBriefCommunicationGrowth2021}. Objective (b) is achieved by formulating a surrogate empirical
model that best approximates the simplified physical model. 

\subsection{Simplified physical model}
...
\subsubsection{Freezing rate to discharge rate}
