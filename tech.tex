...
\begin{abstract}
  ...
  regions. Given scarcity of water resources, optimal AIR construction is needed to reduce dicharge rates while
  realizing maximal meltwater rates. Fountain scheduling methods are promising to improve the water use
  efficiency (WUE) of the contruction process. Two such methods, namely static and dynamic are proposed. The
  dynamic process requires an automation system for real-time fountain scheduling. The automation software uses
  a simplified equation with 6 coefficients that captures the influence of temperature, humidity, wind and solar
  radiation variations on the freezing rate. Historical meteorological data of the site is required to calculate
  these 6 coefficients. The automated AIR had a WUE three times more than the manual AIR. These results support
  the use of the model in practice. 

  During the AIR construction campaigns in Guttannen in 2022, the dynamic fountain scheduling method was
  compared with a unoptimised manual method using two AIRs. The required amount of water for the dynamic AIR was
  applied acoording to the model. Compared to the manual AIR, 87 \% less fountain water was needed for the
  dynamic AIR. Ice volume estimates validated with drone measurements indicate that the dynamic AIR achieved
  higher volumes inspite the reduced discharge rate. However, this volume difference was due to the better design
  of the dynamic fountain which produced a higher spray radius.   

  Overall. our results show that the dynamic fountain scheduling method is attractive given its water use efficiency and automated
  approach.
...
may be incorporated through parametrizations based on time and location. In order to produce recommended
discharge rates, empirical models are preferred due to their parsimony in data requirement in comparison with
the more sophisticated energy-balance models. But only physical energy balance models exist for AIRs. Therefore,
we present a new empirical model that produces fountain scheduling simulations with reduced data requirements
...
\section{Empirical fountain scheduling algorithm}
...
\cite{oerlemansBriefCommunicationGrowth2021}. 
...
relative humidity, wind speed and altitude. $q_{SW}$ can be approximated using a gaussian equation.
...
\subsection{Dynamic fountain scheduler equation }

\subsection{Static fountain scheduler equation }
